    ...
    const callback = (error, results, fields) => {
      if (error) {
         ...
         res.status(500).json(error);
            ...
            if (name){
                return res.status(409).json({message: "Duplicate username is not allowed!"});
            }
         next();
      } 
   }
...
module.exports.updateUserById = (req, res, next) =>{
   if (req.body.username == undefined || req.body.points == undefined || req.body.tickets == undefined){
      res.status(400).send("Error: a or username or points or tickets is undefined");
      return;
   }
    const data = {
        user_id: req.params.id,
